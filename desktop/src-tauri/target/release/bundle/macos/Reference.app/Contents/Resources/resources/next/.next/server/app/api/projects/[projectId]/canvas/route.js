(()=>{var a={};a.id=798,a.ids=[798],a.modules={261:a=>{"use strict";a.exports=require("next/dist/shared/lib/router/utils/app-paths")},10846:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},29294:a=>{"use strict";a.exports=require("next/dist/server/app-render/work-async-storage.external.js")},31421:a=>{"use strict";a.exports=require("node:child_process")},33386:(a,b,c)=>{"use strict";c.d(b,{HE:()=>e,RQ:()=>g,Zg:()=>h,rR:()=>f,wI:()=>i});var d=c(42911);function e(a){let b=(0,d.L)(),c=b.prepare("SELECT * FROM project_sync WHERE project_id = ?").get(a)??null;if(c)return c;let e=new Date().toISOString();return b.prepare(`INSERT INTO project_sync (project_id, canvas_rev, view_rev, canvas_updated_at, view_updated_at)
     VALUES (?, 0, 0, ?, ?)
     ON CONFLICT(project_id) DO NOTHING`).run(a,e,e),b.prepare("SELECT * FROM project_sync WHERE project_id = ?").get(a)??{project_id:a,canvas_rev:0,view_rev:0,canvas_updated_at:e,view_updated_at:e}}function f(a){return(0,d.L)().prepare(`SELECT * FROM canvas_objects
       WHERE project_id = ?
       ORDER BY z_index ASC, created_at ASC`).all(a)}function g(a){let b=(0,d.L)(),c=new Date().toISOString(),e=b.prepare("DELETE FROM canvas_objects WHERE project_id = ?"),f=b.prepare(`INSERT INTO canvas_objects (
      id, project_id, type, asset_id, x, y, scale_x, scale_y, rotation,
      width, height, z_index, props_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),g=b.prepare(`INSERT INTO project_sync (project_id, canvas_rev, view_rev, canvas_updated_at, view_updated_at)
     VALUES (?, 1, 0, ?, ?)
     ON CONFLICT(project_id) DO UPDATE SET
       canvas_rev = canvas_rev + 1,
       canvas_updated_at = excluded.canvas_updated_at`);b.transaction(()=>{for(let b of(e.run(a.projectId),a.objects))f.run(b.id,a.projectId,b.type,b.asset_id??null,b.x,b.y,b.scale_x,b.scale_y,b.rotation,b.width??null,b.height??null,b.z_index,b.props_json??null,c,c);g.run(a.projectId,c,c)})()}function h(a){return(0,d.L)().prepare("SELECT * FROM project_view WHERE project_id = ?").get(a)??null}function i(a){let b=(0,d.L)(),c=new Date().toISOString(),e=b.prepare(`INSERT INTO project_view (project_id, world_x, world_y, zoom, updated_at)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(project_id) DO UPDATE SET
       world_x=excluded.world_x,
       world_y=excluded.world_y,
       zoom=excluded.zoom,
       updated_at=excluded.updated_at`),f=b.prepare(`INSERT INTO project_sync (project_id, canvas_rev, view_rev, canvas_updated_at, view_updated_at)
     VALUES (?, 0, 1, ?, ?)
     ON CONFLICT(project_id) DO UPDATE SET
       view_rev = view_rev + 1,
       view_updated_at = excluded.view_updated_at`);b.transaction(()=>{e.run(a.projectId,a.world_x,a.world_y,a.zoom,c),f.run(a.projectId,c,c)})()}},42911:(a,b,c)=>{"use strict";c.d(b,{L:()=>p});var d=c(73024),e=c.n(d),f=c(76760),g=c.n(f),h=c(87550),i=c.n(h);let j=`PRAGMA foreign_keys = ON;
